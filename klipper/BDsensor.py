...
class BDPrinterProbe:
    def __init__(self, config, mcu_probe):
        ...
        self.sample_retract_dist = config.getfloat(
            "sample_retract_dist", 5.0, above=0.0
        )
        self.samples_result = config.getchoice(
            "samples_result", {"median": "median", "average": "average"}, "average"
        )
        ...
        self.bd_sample_timer = self.reactor.register_timer(self.scan_sample_event)
    ...
    def _probe(self, speed):
        ...
        phoming = self.printer.lookup_object("homing")
        pos = toolhead.get_position()
        self.mcu_probe.reactor.update_timer(
            self.mcu_probe.bd_update_timer, self.mcu_probe.reactor.NEVER
        )
        pos[2] = self.z_position
        try:
            epos = phoming.probing_move(self.mcu_probe, pos, speed)
        ...
        while len(positions) < sample_count:
            ...
            try:
                if (self.mcu_probe is not None) and (
                    ("BED_MESH_CALIBRATE" in gcmd.get_command())
                    or (
                        (
                            "QUAD_GANTRY_LEVEL" in gcmd.get_command()
                            or "Z_TILT_ADJUST" in gcmd.get_command()
                        )
...
class BDsensorEndstopWrapper:
    def __init__(self, config):
        ...
        self.z_adjust = config.getfloat("z_adjust", 0.0, minval=-0.3, below=0.3)
        self.z_offset = config.getfloat("z_offset", 0.0, minval=-0.6, maxval=0.6)
        self.position_endstop = config.getfloat(
            "position_endstop", 0.7, minval=0.5, below=2.5
        )
        if self.z_adjust > self.position_endstop:
            raise self.printer.command_error(
                "The 'z_adjust' cannot be greater"
                " than 'position_endstop' in "
                "section [BDsensor]"
            )
        if self.z_offset > self.position_endstop:
            raise self.printer.command_error(
                "The 'z_offset' cannot be greater"
                " than 'position_endstop' in "
                "section [BDsensor]"
            )
        ...
        self.speed = config.getfloat("speed", 3.0, above=0.0)
        ...
        self.switch_mode = 0
        self.printer.register_event_handler(
            "stepper_enable:motor_off", self.event_motor_off
        )
        ...
        if self.width_filament == 0:
            ...
            self.gcode.register_command(
                "BDSENSOR_READ_CALIBRATION", self.BD_read_calibration
            )
        ...
        self.homing = 0
        self.reactor = self.printer.get_reactor()
        self.bd_update_timer = self.reactor.register_timer(self.bd_update_event)
    ...
    def build_config(self):
        ...
        self.mcu_endstop.add_config_cmd(
            ...
            " endstop_pin=%s sw=0 sw_val=0" % (self.oid_endstop, self.endstop_pin_num),
            on_restart=True,
        )
        # Lookup commands
    ...
    def bd_set_cur_z(self, hgt, s_log):
        if s_log == 1:
            self.gcode.respond_info(
                "Curent z:%.2f (gcode_z:%.2f - (z_offset:%.2f - z_offset_adj:%.2f))"
                % (
                    hgt - (self.z_offset - self.z_offset_adj),
                    hgt,
                    self.z_offset,
                    self.z_offset_adj,
                )
        ...
        if (hgt - self.z_offset) <= 0:
            self.gcode.respond_info(
                "The real_time_adjust is ignored because current z:%.2f (gcode_z:%.2f - (z_offset:%.2f - z_offset_adj:%.2f)) <=0"
                % (
                    hgt - (self.z_offset - self.z_offset_adj),
                    hgt,
                    self.z_offset,
    ...
    def bd_update_z(self, z):
        ...
        for stepper in kin.get_steppers():
            if (
                stepper.is_active_axis("z")
                or stepper.is_active_axis("a")
                or stepper.is_active_axis("b")
                or stepper.is_active_axis("c")
            ):
    ...
    def BD_Sensor_Read(self, fore_r):
        ...
        if fore_r == 0:
            if self.bd_value >= 10.24:
                self.gcode.respond_info(
                    "Bed Distance Sensor " "data error0:%.2f" % self.bd_value
                )
                raise self.printer.command_error(
                    "Bed Distance Sensor " "data error0:%.2f" % self.bd_value
                )
                ...
                self.gcode.respond_info(
                    "Bed Distance Sensor, " "out of range.:%.2f " % self.bd_value
                )
                raise self.printer.command_error(
                    "Bed Distance Sensor, " "out of range.:%.2f " % self.bd_value
                )
            ...
            if self.bd_value >= 10.24:
                self.gcode.respond_info(
                    "Bed Distance Sensor " "data error2:%.2f" % self.bd_value
                )
                raise self.printer.command_error(
                    "Bed Distance Sensor " "data error2:%.2f" % self.bd_value
                )
    ...
    def BD_version(self, gcmd, r_lenght=20):
        ...
        if "V1." in self.bdversion:
            ...
            self.gcode.respond_info(
                "No data or corrupt data from BDsensor(%s), "
    ...
    def BD_calibrate(self, gcmd):
        ...
        if raw_d > 600:
            if raw_d == DATA_ERROR:
                raise self.printer.command_error(
                    "Unable to communicate with bdsensor,%d" % raw_d
                )
            raise self.printer.command_error(
                "BDsensor is too far from the bed:%d" % raw_d
            )
        self.I2C_BD_send(CMD_DISTANCE_MODE)
        ...
        if "z" not in self.toolhead.get_status(curtime)["homed_axes"]:
            raise self.printer.command_error(
                "make sure [force_move]" " enable_force_move: true # in the printer.cfg"
            )
    ...
    def BD_read_calibration(self, gcmd):
        ...
        while 1:
            ...
            if intd < 45:
                gcmd.respond_raw(
                    "BDSensor mounted too close! please mount"
    ...
    def bd_set(self, gcmd):
        ...
        if cmd_bd is not None:
            gcmd.respond_info(
                "z_adjust:%f, recommend to move the nozzle "
        ...
        if cmd_bd is not None:
            ...
            gcmd.respond_info(
                "no_stop_probe is setted:%d The SAVE_CONFIG"
    ...
    def BD_real_time(self, bd_height):
        if bd_height >= 3.0:
            ...
            bd_height = 0
        self.gcode.respond_info(
            "Real time leveling height:%.2f ( %.2f - z_offset:%.2f ) "
            % (bd_height - self.z_offset, bd_height, self.z_offset)
        )
    ...
    def process_M102(self, gcmd):
        ...
        if cmd_bd == -6:
            ...
            strd = (
                strd
                + ",  Then we can calculate the distance by comparing to the calibration data:"
                + str(self.bd_value)
                + "mm"
            )
            gcmd.respond_raw(strd)
            ...
            self.gcode.respond_info(
                "in switch mode, the endstop position is"
    ...
    def home_start(
        ...
        if self.switch_mode == 1:
            ...
            sample_count = 2
            if self.homing == 1 and (
                self.collision_homing == 1 or self.collision_calibrating == 1
            ):
                # self.I2C_BD_send(1)
                collision_value = 1
            # else:
        ...
        for i, trsync in enumerate(self._trsyncs):
            ...
            trsync.start(
                print_time, report_offset, self.trigger_completion, expire_timeout
            )
    ...
    def home_wait(self, home_end_time):
        ...
        self.trigger_completion.wait()
        self._home_cmd.send(
            [self.oid_endstop, 0, 0, 0, 0, 0, 0, 0, self.endstop_pin_num, 0, 0]
        )
        ffi_main, ffi_lib = chelper.get_ffi()
    ...
    def multi_probe_begin(self):
        ...
        if (
            ...
            and self.endstop_pin_num == self.sda_pin_num
        ):
        ...
        while 1:
            ...
            if (raw_d - intr) >= (up_steps * 100 * 2):
                ...
                while 1:
                    ...
                    if (raw_d - intr) >= (second_steps * 100 * 2) or homepos[
                        2
                    ] >= pos_old_1:
                        if logd == 1:
                            temp = 0
                            try:
                                ...
                                temp, target = heaters.get_temp(
                                    self.printer.get_reactor().monotonic()
                                )
                            except Exception as e:
                                pass
                            self.gcode.respond_info(
                                "Raw data:%d at 0 mm, BDsensor to bed: %.4f mm, Bed: %.1fC, z_offset: %.3f"
    ...
    def multi_probe_end(self):
        ...
        if self.endstop_pin_num == self.sda_pin_num:
            ...
            if (
                self.switch_mode == 1
                and self.homing == 1
                and (self.collision_homing == 1 or self.collision_calibrating == 1)
            ):
                ...
                if self.collision_calibrating != 1:
                    ...
                    if abs(self.bd_value - self.z_offset - 0.5) > 0.05:
                        self.gcode.respond_info(
                            "Detect the plate changed or others, the BD sensor needs recalibration %.3f"
                            % (self.bd_value - 0.5)
                        )
                ...
                if self.sda_pin_num is not self.endstop_pin_num:
                    ...
                    self.gcode.respond_info(
                        "offset of endstop to bdsensor %.3fmm"
